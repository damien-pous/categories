...
#[primitive] HB.mixin Record IsPreMonoidal 𝐂 of cat 𝐂 := {
    #[canonical=no] unit: 𝐂;
...
HB.structure Definition premonoidalcat := { 𝐂 of uip 𝐂 & cat 𝐂 & IsPreMonoidal 𝐂 }.

Definition tensor11 {𝐂: PreMonoidalCat} (A B: 𝐂): 𝐂 := app11 tensor A B.
Infix "⊗" := tensor11: cat_scope.
Definition tensor22 {𝐂: PreMonoidalCat} {A B C D: 𝐂} (f: A~>B) (g: C~>D): A⊗C ~> B⊗D := app22 tensor f g.
...
Definition unitl A: unit ⊗ A ~> A := unitl_ (tt,A). 
Definition unitr A: A ⊗ unit ~> A := unitr_ (A,tt).
...
Proof.
  rewrite (UIP_ob _ _ p (tensor11_eq (tensor11_eq a b) c)).
  rewrite (UIP_ob _ _ q (tensor11_eq a (tensor11_eq b c))).
  exact/hcast_assoc.
Qed.
Lemma hcast_assoc' A A' B B' C C' (a: A=A') (b: B=B') (c: C=C'):
  hcast' (assoc' A B C)
...
Lemma hcast_assoc'_ A A' B B' C C' p q (a: A=A') (b: B=B') (c: C=C'):
  hcast' (assoc' A B C) p q = assoc' A' B' C'.
Proof.
  rewrite (UIP_ob _ _ p (tensor11_eq a (tensor11_eq b c))).
  rewrite (UIP_ob _ _ q (tensor11_eq (tensor11_eq a b) c)).
  exact/hcast_assoc'.
Qed.
...
Proof.
  rewrite (UIP_ob _ _ p (tensor11_eq eq_refl a)).
  exact/hcast_unitl.
Qed.
Lemma hcast_unitl' A A' (a: A=A'):
...
Proof.
  rewrite (UIP_ob _ _ p (tensor11_eq a eq_refl)).
  exact/hcast_unitr.
Qed.
Lemma hcast_unitr' A A' (a: A=A'):
...
(** * class for automatic inferrence of McLane isomorphisms  *)
...
(** temporay resolution, to be overriden in [monoidal_gmclane]  *)
